...
public class DashboardUtil {
    ...
    /**
     * Determines the complete directory path to a module in the build, by traversing up the
     * {@link MavenProject} tree. The recursion stops if there is no parent, or if it has arrived at
     * the execution root ({@link MavenProject#isExecutionRoot()}).
     *
    ...
    public static String determineCompletePath(MavenProject mavenProject) {
        if (mavenProject.getParent() != null && mavenProject.getParent().getCollectedProjects().contains(mavenProject)) {
    ...
    /**
     * Execute an XPath function (like count) on a {@link Document}, and return a String containing
     * the result of the function. If the result was a {@link Double}, it returns the integer
     * value.
     *
    ...
    /**
     * Read an ordered list of {@link DashboardReport}s from the properties file passed in, and
     * store them in the reportNames {@link Map} passed in.
     *
     * @param reportNames A {@link Map} containing a mapping from the report name to the
     *            {@link DashboardReport} class name.
